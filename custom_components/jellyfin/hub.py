...
class MediaBrowserHub:
    ...
    async def _async_authenticate(self) -> None:
        # API key authentication - validate the key and get user context
        if not self.api_key:
        ...
        try:
            ...
            if err.status == 401:
                raise aiohttp.ClientResponseError(None, None, status=401, message="Invalid API key") from err
            raise
    ...
    async def _async_rest_post_response(
        ...
        # Add API key as query parameter if available (Jellyfin preferred method)
    ...
    def _auth_update(self) -> None:
        ...
        # Set up authentication headers for API key
        if self.api_key is not None:
            # For Jellyfin (and also works for Emby)
            auth = (
        ...
        # Setup websocket URL based on server type (if known)

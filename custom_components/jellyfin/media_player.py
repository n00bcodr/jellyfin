    ...
    async def session_changed(
        ...
        if (
            ...
            if player_entity := next(
                ...
                # Remove the device associated with this session
                session_device_id = (DOMAIN, old_session[Session.ID])
                if device := device_registry.async_get_device(identifiers={session_device_id}):
                    _LOGGER.debug("Purging device %s", device.name)
...
class MediaBrowserPlayer(MediaBrowserEntity, MediaPlayerEntity):
    ...
    async def async_send_message(self, text: str, header: str, timeout: int | None = None) -> None:
        """Send a message to the media player session."""
        if self._session is not None:
            data = {
                "Text": text,
                "Header": header,
            ...
            if timeout is not None:
                data["TimeoutMs"] = timeout * 1000  # Convert seconds to milliseconds
            await self.hub.async_command(self._session_key, "DisplayMessage", data=data)

    async def async_send_command(self, command: str, arguments: dict[str, Any] | None = None) -> None:
        """Send a command to the media player session."""
        if self._session is not None:
